...
class hk_packet_cls(NamedTuple):
    ...
    def from_bytes(cls, bytes_: bytes) :
        structure = struct.unpack(packet_format_hk, bytes_)   
  	                                     # Timestamp- bit15: Telemetry Type=1,  Mask what type of telemetry packet you have and
        if not structure[1] & 0x80000000:  # Only process hk packets here, Telemetry Type = 0 are science packets, we want to process hk and science packets differently
            return None                      # Should there be two files, one for housekeeping and one for science?  I don't know the answer to this

        return cls(
            timestamp=structure[1] & 0x3fffffff,       # mask for getting all timestamp bits --We can discuss masking by teams or zoom if you would like too--
            hk_status=(structure[2] & 0xf000) >> 12,   # down shift the hk ID bits -I would call hk_status hk_identifier or hk_id so it is the same as LEXI specification document
            hk_value=(structure[2] & 0xfff) << 4,      # upshift the 12 bits of hk data -- the Analog to Digital Converter is 16bits, but only the 12msb are sent down
            delta_event_count=structure[3],            # So you up shift them 4 bits and fill with zeros, we lose 0.00006881 * 15 = 1mVolt of accuracy on any hk datapoint
            delta_drop_event_count=structure[4],       # but this allows us to have smaller packets and get more science (we keep all 16 bits) to the ground.
...
def read_binary_data_hk(
    ...
    for ii,hk_packet in enumerate(packets):
        ...
        if hk_packet.hk_status==0:                       # I would change the hk_status to hk_id everywhere
            PinPullerTemp[ii] = (hk_packet.hk_value - 2.73) * 100    #  I don't see the original ADC counts being converted to voltages first, then equations applied
